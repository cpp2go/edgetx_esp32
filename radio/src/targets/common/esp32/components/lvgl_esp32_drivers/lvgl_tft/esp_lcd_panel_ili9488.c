...
#include "ili9488.h"
#ifdef CONFIG_SPIRAM
#include "esp32s3/rom/cache.h"  // Cache_WriteBack_Addr: flush CPU D-cache to PSRAM before GDMA reads
#endif
...
static esp_err_t panel_ili9488_init(esp_lcd_panel_t *panel)
{
    ...
    ESP_LOGI(TAG, "panel_ili9488_init free_internal=%d free_total=%d", esp_get_free_internal_heap_size(), esp_get_free_heap_size());

    dmabuf = (uint8_t *) heap_caps_malloc(DISP_BUF_SIZE * sizeof(lv_color16_t), MALLOC_CAP_DMA);
    if (dmabuf == NULL) {
        ESP_LOGW(TAG, "DMA alloc failed (%d bytes), falling back to internal", DISP_BUF_SIZE * sizeof(lv_color16_t));
        dmabuf = (uint8_t *) heap_caps_malloc(DISP_BUF_SIZE * sizeof(lv_color16_t), MALLOC_CAP_INTERNAL);
    }
    if (dmabuf == NULL) {
        ESP_LOGW(TAG, "Internal alloc failed, falling back to heap");
        dmabuf = (uint8_t *) malloc(DISP_BUF_SIZE * sizeof(lv_color16_t));
    }
    ESP_ERROR_CHECK(dmabuf == NULL ? ESP_ERR_NO_MEM : ESP_OK);
    return ESP_OK;
}
...
#ifdef CONFIG_SPIRAM
    // dmabuf may be in PSRAM (internal alloc fails after BLE init for 38400-byte buffer).
    // memcpy writes via CPU D-cache; GDMA reads PSRAM directly → stale data without this flush.
    Cache_WriteBack_Addr((uint32_t)dmabuf, (uint32_t)len);
#endif
