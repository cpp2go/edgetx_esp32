...
static void es8311AudioInit(void)
{
    ...
    // DAC volume. 0xBF (~0 dB) is the Espressif driver's top-of-range default;
    // EdgeTX applies its software volume on top. (The previous -9 dB cut was a
    // workaround for the U16-vs-S16 DC bias which made audio over-loud; with
    // the signed-sample fix in board.h the full-scale level is correct again.)
    es8311_write_reg(0x32, 0xBF);   // DAC volume 0 dB (software volume on top)
